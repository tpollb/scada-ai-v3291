...
### Документация
- Создан `backend/docs/auth.md` — полная документация по системе авторизации и ролевой модели:
  - Ролевая модель (4 роли, матрица доступов)
  - Архитектура авторизации (схема потока аутентификации)
  - Механизм работы (логин, токены, сессии, выход, восстановление)
  - Ролевая защита на бэкенде (AuthMiddleware, require_role)
  - Ролевая защита на фронтенде (скрытие UI, hasRole)
  - API Endpoints с примерами запросов
  - Управление пользователями (хранилище, смена паролей)
  - Лицензирование и авторизация (связь двух систем)
  - Защита от краевых случаев (истёкшие токены, 401/402/403)
  - Примеры кода (безопасные хелперы)
  - Известные проблемы и решения (история багов)
  - Быстрый старт (демо-доступы)
- `auth.md` добавлен в whitelist `docs.py` для отображения в конфигураторе
